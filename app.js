...
function getCountryByIso3(isoCode){
    ...
    const res = countries.find((element) => element.iso3 === isoCode)
    return(res)
}
...
function getCountryTranslatedName(isoCode, language){
    //Complete with your code
    const res1 = countries.find((element) => element.iso3 === isoCode)
    const res2 = res1.translations[language]
    return(res1, res2)
}
...
function getCountriesBySubregion(subregion){
    ...
    countries.forEach( country => {
        if(country.subregion === subregion){
            countryRegion.push(country.name)
        }
    })
